...
const shops = [
    {
        ...
        searchItem: async ({ browser, url, item }) => {
            ...
            for (let index = 0; index < 5; index++) {
                ...
                let titleItem = await page.$eval('#title', el => el.textContent)
                let imgItem = await page.$eval('#imgTagWrapperId > img', el => el.src)
                let priceItem
                try {
                    priceItem = await page.$eval('.aok-offscreen', el => el.textContent)
                } catch (error) {
